...
if (isset($_POST['id'])) {
    ...
    $firstname = $lastname = $birthday = $sex = $street = $barangay = $city = $province = $user_id = '';
    ...
    if(isset($_POST['update'])) {
        ...
        if ($con->updateUser($user_id, $firstname, $lastname, $birthday, $sex)) {  
            if ($con->updateUserAddress($street, $barangay, $city, $province, $user_id)) {
  ...
  <form method="POST">
    ...
    <div class="row justify-content-center gx-0">
        ...
        <input type="hidden" name="id" value="<?php echo $data['user_id'];?>" >
            <input type="submit" name="update" value="Update" class="btn btn-outline-primary btn-block mt-4" value="Update">
